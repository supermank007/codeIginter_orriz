<?php if ($posts != null) {
    foreach ($posts as $rows) {
        ...
        if (isset($rows['last'])) {
            ...
            $last_mobile = "added {$rows['last']} ago";
        } else {
            $last = "Post added moment ago";
        }
//        if ($rows['video'] != null) {
//            $video = parse_url($rows['video']);
//            if ($video['host'] == 'www.dailymotion.com') {
//                $host = 'www.dailymotion.com';
//                $path = $video['path'];
//            } elseif ($video['host'] == 'www.youtube.com') {
//                $host = 'www.youtube.com';
//                $path = array_pop(explode("=", $video['query']));
//            } elseif ($video['host'] == 'youtube.com') {
//                $host = 'youtube.com';
//                $path = array_pop(explode("=", $video['query']));
//            } elseif ($video['host'] == 'youtu.be') {
//                $host = 'www.youtube.com';
//                $path = array_pop(explode("/", $video['path']));
//            } elseif ($video['host'] == 'dai.ly') {
//                $host = 'www.dailymotion.com';
            ...
            <div class="ofv oln ptl pbxs">
                ...
                <div class="orriztime-media-body">
                    <div class="orriztime-container">
                        <a class="orriztime-user-name ng-binding" style="font-size: 13px"
                           href="<?= $rows['member_url'] ?>"><?php echo $rows['first_name'] . '  ' . $rows['last_name']; ?></a>
                        <div style="font-size:12px; line-height: 12px" class="visible-xs">
                            <?= date($rows['time']) ?>
                        </div>
                        <span class="time-ago visible-xs"><?= $last_mobile ?></span>
                        ...
                        <?php } ?>
                        <?php if (($rows['video']) != null) { ?>
                            <div class="video">
                                <iframe width="300" height="169" src="<?= $rows['video'] ?>"
                        ...
                        <div class="orriztime-actions">
                            ...
                            <span class="bar hidden-xs"></span>
                            <span class="time-ago hidden-xs"><?= $last ?></span>
                        </div>
                    </div>
