...
<div class="container">
    <div class="myprofile-box">
            ...
            <div class="col-sm-12">
                <div id="carousel-example-generic" class="carousel slide wallpaper_slider" data-ride="carousel"
                    ...
                    <ol class="carousel-indicators">
                        <?php //var_dump($friend_request_sent);exit;
        ...
        <!-- Slider Code End-->
        <?php
        $friendField = "Add Friend";
        $elementId = 'addFriend';
        $elementStyle = '';
        if ($isFriend) {
            $friendField = "Unfriend";
            $elementId = 'deleteFriend';
        }
        if ($isFriendRequestSent) {
            $friendField = "Friend request sent";
            $elementId = '';
            $elementStyle = 'style="cursor: default; color: #428bca;"';
        }
        ?>
        ...
        <div class="row">
            <div class="col-sm-6">
                        ...
                        <div class="referral-content-container">
                            <div class="referral-number">
                                <ul class="list-group">
                                    ...
                                    <? //= base_url() ?><!--members/edit_profile">Edit Profile</a></li>-->
                                    <!--                                    <li class="list-group-item"><a href="-->
                                    <? //= base_url() ?><!--images/gallery">Edit DP</a></li>-->
                                    <!--                                    <li class="list-group-item"><a class="edit_slider_wallpaper" style="cursor:pointer;">Edit Slider</a></li>-->
                                    <li class="list-group-item"><a <?= $elementStyle ?> id="<?= $elementId ?>"
                                                                                        href="javascript:void(0);"><?= $friendField ?></a>
                                    </li>
                                    <li class="list-group-item"><a friendid="<?= $users_data[0]['id'] ?>"
                                                                   fname="<?= $users_data[0]['first_name'] ?>"
                                                                   lname="<?= $users_data[0]['last_name'] ?>"
                                                                   member_url="<?= $users_data[0]['member_url'] ?>"
                                                                   href="javascript:void(0);"
                                                                   class="send-message-user">Send
                                            Message</a></li>
                                </ul>
                            </div>
                        </div>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-8">
                        ...
                        <?php if ($me->id != $users_data[0]['id']) { ?>
                            <div style="top: -40px; right: 50px; position: absolute;">
                ...
                <div class="row">
                    ...
                    <?php if (isset($users_friend_list)) { ?>
                        <div class="col-xs-12">
                            <div class="row row-no-padding">
                                <?php foreach ($users_friend_list as $rows) {
                                                ...
                                                <?php if ($rows['id'] != $myid) { ?>
                                                    <div class="user-name-icon">
                                                        <a id="<?php echo $rows['id']; ?>" href="javascript:void(0);"
                                                           class="btn_add_friend"><i class="fa fa-user-plus"
                                                                                     aria-hidden="true"
                                                                                     title="Add friend"></i></a>
                                                    </div>
                                                    <div class="user-name-message">
                                                        <a friendid="<?= $rows['id'] ?>"
                                                           fname="<?= $rows['first_name'] ?>"
                                                           lname="<?= $rows['last_name'] ?>"
                                                           member_url="<?= $rows['member_url'] ?>"
                                                           href="javascript:void(0);"
                                                           class="msg-system send-message-user"><i
            ...
            <div class="col-sm-6">
                <div class="referral-content-container">
                    <div class="referral-number">
                        <h3><?php if (time() - strtotime($users_data[0]['last_seen']) < 600 && $users_data[0]['is_hidden'] == NULL) { ?>
                                <img src="<?php echo base_url(); ?>public/images/online_green_button.png">
                            <?php }
                            echo ucfirst($users_data[0]['first_name']) . ' ' . ucfirst($users_data[0]['last_name']); ?>
                        </h3>
                        <table style="width:100%;">
                            ...
                            <tr>
                                <td>Last Active:</td>
                                <td>:</td>
                                <td><?php if (time() - strtotime($users_data[0]['last_seen']) < 600 && $users_data[0]['is_hidden'] == NULL) {
                                        echo 'Online Now!';
                                    } elseif (time() - strtotime($users_data[0]['last_seen']) < 600 && $users_data[0]['is_hidden'] != NULL) {
                                        echo $users_data[0]['is_hidden'] . " ago";
                                    } else {
                                        echo $users_data[0]['last_active'] . " ago";
                                    } ?></td>
                            </tr>
                            <!--                            <tr>-->
                            <!--                                <td>Profile Views</td>-->
                            <!--                                <td>:</td>-->
                            <!--                                <td>-->
                            <? //= $users_data['count_views'] ?><!-- times</td>-->
                            <!--                            </tr>-->
                            <tr>
                                <td>Member Since</td>
                                <td>:</td>
...
<script>
    ...
    function addFriend() {
        ...
        $('#addFriend').off('click').html('Friend request sent').attr('style', 'cursor: default; color: #428bca;');
    }

    $('#deleteFriend').click(function () {
        var friend_id = <?= $users_data[0]['id'] ?>;
        $.ajax({
            url: "<?= base_url('dashboard/unfriend') ?>",
            cache: false,
            data: {
                friend_id: friend_id,
            },
        ...
        $(this).off('click').on('click', addFriend).html('Add Friend').attr('id', 'addFriend');
    });
</script>
</body>
</html>