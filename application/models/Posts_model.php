...
class Posts_model extends CI_Model
{
    ...
    public function count_rows_privacy1($friend_list = array())
    {
        ...
        $query = $this->db->query("SELECT posts.id,posts.status,posts.photos, posts.video, posts.time ,members.first_name,members.last_name, members.image, COUNT(posts_like.id) as likes, GROUP_CONCAT(members.first_name SEPARATOR '|') AS liked_by from posts left JOIN posts_like ON posts.id=posts_like.post_id left join members on members.id=posts.member_id where posts.member_id in($friend_list) and (posts.privacy in (1, 2, 3)) Group by posts.id order by posts.id DESC");
    ...
    {
        $friend_list = join(', ', $friend_list);
        $query = "SELECT posts.id,posts.status,posts.photos, posts.video, posts.time ,members.first_name,members.last_name, members.image, COUNT(posts_like.id) as likes, GROUP_CONCAT(members.first_name SEPARATOR '|') AS liked_by from posts left JOIN posts_like ON posts.id=posts_like.post_id left join members on members.id=posts.member_id where posts.member_id in($friend_list) and posts.privacy=2  Group by posts.id order by posts.id DESC";
    ...
    {
        $q = "SELECT posts.id,posts.status,posts.photos, posts.video, posts.time ,members.first_name,members.last_name, posts.member_id,  members.profile_url, members.image, COUNT(posts_like.id) as likes, GROUP_CONCAT(members.first_name SEPARATOR '|') AS liked_by from posts left JOIN posts_like ON posts.id=posts_like.post_id left join members on members.id=posts.member_id where posts.privacy =1 AND posts.show_in_posts=1  Group by posts.id order by posts.id DESC Limit $start, $limit";
    ...
    public function select_rows_privacy2($start, $limit, $friend_list)
    {
        ...
        $q = "SELECT posts.id,posts.status,posts.photos, posts.video, posts.member_id, posts.time ,members.first_name,members.last_name,  members.profile_url, members.image, COUNT(posts_like.id) as likes, GROUP_CONCAT(members.first_name SEPARATOR '|') AS liked_by from posts left JOIN posts_like ON posts.id=posts_like.post_id left join members on members.id=posts.member_id where posts.privacy=2 AND posts.show_in_posts=1 AND posts.member_id IN($frndListt) Group by posts.id order by posts.id DESC Limit $start,$limit";
        $query = $this->db->query($q);
        return $query->result_array();
    }
