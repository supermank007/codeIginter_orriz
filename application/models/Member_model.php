...
class Member_Model extends CI_Model
{
    ...
    {
        $this->db->select(['id', 'first_name', 'last_name', 'image', 'profile_url', 'luv_points', 'points', 'is_hidden']);
        $query = $this->db->get_where('members', ['id' => $id]);
        return $query->result();
    }
...
$limit $offset");
    ...
    public function getRelatedUsers()
    {
        ...
        $res = [];
        $ids = $this->getFriendsIds() . $this->getUsersNotShow();
        $count = 0;
        if ($user->city) {
        ...
        if ($count < $limit) {
            $newLimit = $limit - $count;
            $query = $this->db->query("SELECT * FROM `members` WHERE `id` NOT IN($ids) ORDER BY rand() LIMIT $newLimit");
...
//                $ids[] = $friend['id'];
    ...
    private function getUsersNotShow()
    {
        $UserID = $this->session->userdata('user_id');
        $this->db->select(['this_member_id']);
        $query = $this->db->get_where('not_show_user', ['for_member_id' => $UserID]);
        $usernotshow = $query->result_array();
        $iduns = "";
        if (!empty($usernotshow)) {
            foreach ($usernotshow as $notshow) {
                $iduns .= ", {$notshow['this_member_id']}";
            }
        }
        return $iduns;
    }

    public function notShowUser($id)
    {
        return $this->db->insert('not_show_user', ['this_member_id' => $id, 'for_member_id' => $this->session->userdata('user_id')]);
    }
    ...
    {
        $query = $this->db->query("SELECT F.status,M.id, M.profile_url, M.first_name, M.last_name, M.image, M.city, M.country, M.birthday, M.last_seen, M.is_hidden 
                            FROM members M, friends F WHERE CASE
    ...
    public function isFriend($user_id) {
        $UserID = $this->session->userdata('user_id');
        $query = $this->db->query("SELECT status FROM friends WHERE (friend_one='$user_id' AND friend_two='$UserID') OR (friend_one='$UserID' AND friend_two='$user_id')" );
        return $query->result_array();
    }

    public function isOnline($id)
    {
        $sql = "SELECT M.id, M.profile_url, M.first_name,M.last_name,M.image
                            FROM members M
                            WHERE M.last_seen BETWEEN (DATE_SUB(UTC_TIMESTAMP(),INTERVAL +1 MINUTE)) AND UTC_TIMESTAMP()
                            AND M.id = '$id'
        ;";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
    ...
    {
        $sql = "SELECT F.status,M.id, M.profile_url, M.first_name,M.last_name,M.image, M.city, M.country, M.birthday, M.last_seen, M.is_hidden
    ...
    {
        $id = (int)$id;
        $sql = "UPDATE members SET last_seen = UTC_TIMESTAMP() WHERE id = '" . $id . "' ;";
        $results = $this->db->query($sql);
        if ($results) {
            return true;
        }
    ...
    public function add_visited_member($id)
    {
        return $this->db->insert('visits_members', ['member_id' => $this->session->userdata('user_id'), 'member_visited_id' => $id]);
    }

    public function count_of_views($id)
    {
        $this->db->select(['id']);
        $this->db->where(['member_visited_id' => $id]);
        return $this->db->count_all_results('visits_members');
    }
}