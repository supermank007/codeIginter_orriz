...
//            if($this->uri->segment(1) != "Roulette_controller") {
            redirect(base_url('dashboard'), 'refresh');
//            }
            //redirect(base_url('Roulette_controller'));
        } else {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($user_id != false) {
                if ($this->ion_auth->login($this->input->post('email'), $this->input->post('password'))) {
                    $data = ['is_online' => 1];
                    $this->member_model->update_members_profile($this->session->userdata('id'), $data);
    ...
    public function login()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('login_password'), $remember)) {
                ...
                $_SESSION['faggot'] = $this->ion_auth->login($this->input->post('identity'));
                $_SESSION['faggot2'] = $this->input->post('login_password');
                $_SESSION['usernameE'] = $this->session->userdata('name');
                $_SESSION['identityE'] = $this->session->userdata('identity');
                ...
                // redirect them back to the login page
                $this->session->set_flashdata('message', $this->ion_auth->errors());
                echo $this->load->view('public/index', ['uri' => $uri], true);
                exit;
            }
        } else {
            ...
            $this->data['uri'] = $uri;
            echo $this->load->view('public/index', $this->data, true);
            exit;
        }
    }
