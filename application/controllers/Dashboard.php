...
//        $this->data['is_hidden'] = $this->me->is_hidden;
//        $this->data['points'] = $this->me->points;
//        $this->data['active'] = $this->me->active;
//        $this->data['privacy'] = $this->me->privacy;
//        $this->data['friend_list'] = $this->member_model->friend_list($this->session->userdata('user_id'));
    ...
    private function addGlobalData()
    {
        ...
        $this->data['messengerList'] = $this->messenger_model->getMessengerList();
        foreach ($this->data['messengerList'] as $user) {
...
//        $this->data['ads'] = $this->ad_model->get_latest_10_ad();
    ...
    public function not_show_user_ajax()
    {
        $this_member_id = $this->input->post('member_id');
        if ($this_member_id && $this->member_model->notShowUser($this_member_id)) {
...
//        }
    ...
    public function myprofile()
    {
        ...
        $this->data['users_data']['count_views'] = $this->member_model->count_of_views($user_id);
...
//        echo 1;        
    ...
    {
        $friend_two = $_REQUEST['friend_id'];
        $to = $this->member_model->get_email($friend_two)[0]['email'];
        $friend_one = $this->session->userdata('user_id');
        if (!$this->member_model->isOnline($friend_two)) {
            $data['user'] = $this->member_model->get_user($friend_one)[0];
            $subject = "New friend request on Orriz";
            $body = $this->load->view('public/email/sentrequest', $data, true);
...
//        }
    ...
    public function friendrequests()
    {
        ...
        $this->addGlobalData();
        $this->notifications_model->updateNotifications('friends');
        $this->data['notSeenCount']->friends = 0;
    ...
    public function onlinefriends()
    {
        ...
        $online_friend_list = $this->member_model->getOnlineFriends($user_id);
        if ($online_friend_list != null) {
            for ($i = 0; $i < count($online_friend_list); $i++) {
                $online_friend_list[$i]['member_url'] = parent::makeMemberUrl($online_friend_list[$i]['id'], $online_friend_list[$i]['profile_url']);
            }
            $this->data['online_friend_list'] = $online_friend_list;
        } else {
