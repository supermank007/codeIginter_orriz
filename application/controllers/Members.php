...
//    }
    ...
    {
        $data = ['id' => $this->me->id, 'activation' => $this->me->activation_code, 'email' => $this->me->email];
        $to = $this->me->email;
...
//        $currentGroups = $this->ion_auth->get_users_groups($id)->result();
        ...
        $this->data['points'] = $this->data['getStep1ProfileDetails']->points;
        $this->data['is_hidden'] = $this->data['getStep1ProfileDetails']->is_hidden;
...
//    }
    ...
    public function iamhidden()
    {
        ...
        $id = $this->session->userdata('user_id');
        $is_hidden = NULL;
        if ($this->input->post('is_hidden') == 'invisible') {
            $is_hidden = date("Y-m-d H:i:s");
        }
    ...
    public function get_user_for_node()
    {
        echo json_encode(['user' => $this->me]);
        exit;
    }
