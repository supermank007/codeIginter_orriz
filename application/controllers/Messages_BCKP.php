...
//                'sender_id' => $this->input->post('sender_id'),
            ...
            if (!$this->member_model->isOnline($this->input->post('receiver_id'))) {
                $to = $this->member_model->get_email($this->input->post('receiver_id'));
                foreach ($to as $t) {
                    $subject = "Message from " . $this->me->first_name . ' ' . $this->me->last_name;
                    $body = $this->load->view('public/email/message', $data, true);
                    parent::sendEmail($t['email'], $subject, $body);
                }
            }
            if ($is_redirect != 'no') {
    ...
    {
        $receiverId = intval($this->input->post()['receiverId']);
        if (!$receiverId) {
