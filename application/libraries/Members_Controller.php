...
//        $this->load->library('email');
        if ($this->ion_auth->logged_in() == false) {
            $data = [];
            $data['uri'] = $uri;
            echo $this->load->view('public/index', $data, true);
            exit;
        }
...
//        $this->config->set_item('active', $this->me->active);
    ...
    protected function intervalTime($time)
    {
        $now = new DateTime();
...
//        $this->email->from('evgeniypasko@gmail.com', 'Orriz');
//        $this->email->to($to);
...
//        }
    ...
    protected function notifyNode($data)
    {
        $ch = curl_init();
        ...
        curl_setopt($ch, CURLOPT_PORT, 9246);
